...
- **2026-05-04** — LOD threshold tuning queued for 1.2 (recorded in release.md → Next steps). Decision: nudge thresholds in `GridEngine.LODLevel.level(for:)` up one tier so the 50m base detail persists longer at zoom-out, rather than exposing a user toggle. Toggle was considered but rejected on performance grounds (full-detail rendering at state-level zoom on heavy data risks stutter/freeze, which is precisely why LOD exists). Tuning the default is lower-risk, zero-maintenance, and recoverable — if heavy-data users later complain, the toggle becomes the escape hatch.
