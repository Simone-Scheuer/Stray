...
- **2026-05-01** — Added location upgrade banner on the map view, visible when `locationService.authorizationStatus == .authorizedWhenInUse`. Sits below the identity-header row, dark translucent rounded panel, italic-serif explanation + roman-amber action ("Stray only records when the app is open." / "Allow background tracking →"). Tap calls `requestAlwaysPermission()` first time (per `@AppStorage("hasRequestedAlwaysPrompt")`), opens Settings on subsequent taps. Reasoning: real-user discovery gap — iOS won't show the Always upgrade prompt unless app demonstrably uses background location, so users who grant When In Use during onboarding and never open the app long enough never see the prompt. Banner makes the upgrade discoverable without nagging modals.
- **2026-05-04** — Banner widened to also cover `.denied`. Originally only `.authorizedWhenInUse`; missed the case where a user with already-revealed cells denies/revokes location and gets neither the banner nor the big `locationDeniedOverlay` (which only fires when revealedCells is empty). Now suppresses when the big overlay would show, otherwise shows a denied-specific copy ("Location is turned off." / "Turn it on in Settings to keep revealing the map"). Tap routes straight to Settings since iOS won't re-prompt.
- **2026-05-04** — Banner action copy refined: "while" not "when" ("Stray only records *while* the app is open."), and the action line now tells the user what they *get*, not what they're permitting ("Allow background tracking to record even when closed"). Original was descriptive without being motivating.
- **2026-05-04** — Splash wordmark switched from `.system(size: 32, weight: .light, design: .default).tracking(8)` capital `Stray` to `.system(size: 32, weight: .regular, design: .serif).italic()` lowercase `stray`. Splash now mirrors the in-app brand wordmark instead of carrying its own register. Trades the geometric-tracked contrast for brand unity. If the splash reads visually quieter on actual hardware, easy bump to 48pt; left at 32pt for now.
- **2026-05-04** — Cell Inspector summary reverted from italic-serif literary line to plain stat block ("X visits" 22pt regular serif + "last visited Y" 13pt regular serif at 0.6 opacity). The literary phrasing ("You've passed through here three times since the 14th of March, most recently a week ago") read overcooked on a per-tile inspector — affect rather than information. Italic-for-voice discipline preserved elsewhere; the inspector body is now in the function register. First-tester feedback (Elena) flagged the page as "tall and blank" on sparse tiles; coordinate/cell-key density work queued for 1.1.1 if it remains an issue post-launch.
- **2026-05-04** — Timeline panel now always reserves the 52pt photo-strip height regardless of whether the day has photos. Previously the panel resized between days, creating a layout jump and (worse) the photo-having version was tall enough to cover the Apple Maps legal label. Map's `bottomInset` when timeline active also bumped 220 → 305 to clear the now-consistent panel height plus its shadow falloff (radius 20).
- **2026-05-04** — LOD plan revised after testing on real device (see release.md → Next steps for full plan). Initial proposal was "just nudge thresholds outward" — superseded after observing screenshots: the actual cliff is structural. `block`→`district` is 16x linear / 256x area while every other tier transition is 4x. Threshold tuning alone moves the cliff but doesn't kill it. Revised plan is three-part: (1) add an intermediate `street` tier (800m) to make the LOD ladder uniformly 4x-spaced, (2) push thresholds outward so base/block persist longer, (3) ship an opt-in `Settings → Experimental` toggle that bypasses LOD entirely with a clear performance warning. Toggle was reconsidered after the user reframed it as a labeled escape hatch with eyes-open performance trade — different proposition than a default. Optional floor: cap visible cells at ~100k per render even with toggle on, to prevent pathological hangs. (1)+(2) is the feel fix; (3) is opt-in surface for power users.
- **2026-05-05** — LOD heat-mode bugs identified post-launch (1.0/1.1 carryover, not introduced by 1.1). (a) Heat gradient was keyed off coverage instead of visits because `aggregatedCells` returned cell-count as the third tuple field, which the renderer fed straight into `coverageHeatColor`. Result: a fully-covered LOD region of 1-visit cells colored identically to a fully-covered region of 50-visit cells. (b) Per-cell border (the thin "white" outline that shows up at base zoom because heat tints overlay a default revealedCellTint pass) disappeared at LOD because the LOD renderer skipped the base pass and went directly to clear→heat-fill. **Fix:** `aggregatedCells` now returns both `cellCount` and `valueSum` so heat mode can compute `avgVisits = valueSum / cellCount`; LOD heat-mode rendering now does default-tint base pass + heat overlay at `insetBy(0.5, 0.5)`, restoring the border and using existing discrete `heatColor(for:)` tiers for color so it matches base zoom. Unused `coverageHeatColor` removed. **Decided 2026-05-05:** rolled into 1.2 (not shipped as 1.1.1). Reasoning: the bug isn't introduced by 1.1 and isn't crashing anyone; bundling with the structural LOD overhaul means one release, one review cycle, one set of test screenshots. Fix lives on `1.2-lod` branch as its first commit.
- **2026-05-05** — LOD threshold targets pushed further out per first-tester intuition (originally ~0.20° for `base`, now targeting ~0.40°+). Goal phrased by user: "you should be able to look at your whole map at city level and still see your trails." Specifically car-trip trails are the canary — they read as cute thin lines to the eye but get aggregated away too early. Final values need on-device tuning; the recorded targets are intent, not gospel.
- **2026-05-10** — Timeline ergonomics for 1.2.0 build 27. Added back/forward chevron buttons flanking the date in the timeline header — tap to step one day, press-and-hold to auto-advance (350ms activation delay, then 220ms repeat interval). Light haptic on press-down only, not on repeats. Stutter on scrub was traced to `onSelect` firing every 100ms during drag, each call triggering `selectIndex` → day apply → fog tile re-render + photo fetch + pedometer async query. Fix: defer `onSelect` to `onEnded` only; during drag, only the local `dragIndex` updates so the date label re-renders live but the engine work runs once on commit. Loses the "photos fly by during scrub" feeling, gains a buttery drag. If first testers miss the live preview we can split — date label live, engine work deferred.
- **2026-05-10** — LOD tower restructured for 1.2.0 build 26. Original three-part plan called for keeping `block` AND adding `street`, but device testing revealed that `block` (200m, 4x base) is visually indistinguishable from `base` at the zoom levels where it activates — wasting a tier without smoothing the transition. **Final tower:** `base` (50m), `street` (800m, 16x), `district` (3.2km, 4x), `city` (12.8km, 4x), `metro` (~51km, 4x), `region` (~205km, 4x). One 16x jump at the bottom of the ladder (base→street), but it lands at zoom levels where base cells are sub-pixel anyway, so visually the transition feels softer than the old block→district 16x cliff (which kicked in earlier). **Thresholds** also pushed dramatically: `base` now persists to 0.30° (was 0.06°), `street` to 1.5°, `district` to 5.0°, etc. — so "whole-city" zoom keeps the 50m spider-web visible. **Experimental "show all detail" toggle** shipped in Settings (under new `experimental` section) — reads `Constants.experimentalDisableLODKey`, forces `level = .base` in FogOverlay when on, gated by perf-warning footer copy. Wired through MapViewRepresentable → Coordinator → `fogRenderer.setNeedsDisplay()` so toggling triggers immediate redraw.
